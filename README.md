...
```sql
SELECT albums.album_name, albums.artist_name, AVG(songs.rating) AS avg_rating
FROM albums
JOIN songs
ON albums.id = songs.album_id
...
JOIN songs
ON albums.id = songs.album_id
WHERE (SELECT AVG(songs.rating) FROM songs WHERE songs.album_id = albums.id) < songs.rating;
```
